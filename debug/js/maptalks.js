...
(function (global, factory) {
    ...
    var ToolTip = function (_UIComponent) {
      ...
      _proto.buildOn = function buildOn() {
        ...
        if (options$m.height) {
          dom.style.height = options$m.height + 'px';
        }

        if (options$m.width) {
          dom.style.width = options$m.width + 'px';
        }

        var cssName = this.options.cssName;

        if (!cssName && options$m.height) {
          dom.style.lineHeight = options$m.height + 'px';
        }
    ...
    typeof console !== 'undefined' && console.log && console.log('maptalks v0.42.1');
